...
if maxorder_smoother == 1:  # Linear smoother
    ...
    nonmonotone_BWS = [
        [[], [0]]
        + [[0] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)]
        + [[1]]
        + [[1] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)]
        + [[2]]
        + [[2] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)],
        [[], [0]]
        + [[0] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)]
        + [[1]]
        + [[1] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)]
        ...
        + [[2] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)]
        + [[3]]
        + [[3] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)],
        [[], [0]]
        + [[0] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)]
        + [[1]]
        + [[1] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)]
        ...
        + [[3] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)]
        + [[4]]
        + [[4] * od + ["HF"] for od in np.arange(1, maxorder_smoother + 1, 1)],
    ]
...
for t in range(T - 2, -1, -1):
    ...
    )  # Next D dimensions:    filtering analysis
    ...
    ret = tm.inverse_map(X_star=X_star, Z=Z_pushforward)
...
plt.plot(
    np.flip(RMSE_list_smoother),
    color="xkcd:orangish red",
    label="Ensemble Transport Smoother",
)
plt.xlabel("timestep")
plt.ylabel("ensemble mean RMSE")
plt.legend()
plt.title(
    ...
    + str(maxorder_filter)
    + " | RMSE: "
    + f"{np.mean(RMSE_list):.3f}"
    + " | EnTs order "
    + str(maxorder_smoother)
    + " | RMSE: "
    + f"{np.mean(RMSE_list_smoother):.3f}"
)
plt.savefig(
    "02_RMSE_EnTS_order="
    + str(maxorder_filter)
