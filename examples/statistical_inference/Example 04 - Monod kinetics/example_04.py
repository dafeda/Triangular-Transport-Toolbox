...
for idx, _line in enumerate(lines):  # Go through each line string
...
plt.plot(
    C, obs_rate, marker="x", color="xkcd:orangish red", zorder=10, label="observed"
)
...
for k in np.arange(D - 2, D, 1):
    ...
    # Otherweise, the order is greater than one. Let's use special terms.
        ...
        for _order in range(maxorder - 1):
...
tm = transport_map(
    ...
    monotonicity="separable monotonicity",  # Monotonicity method
...
# of the observations.
X_star = np.repeat(a=obs_rate[np.newaxis, :], repeats=N, axis=0)
...
for n in range(N):
    pred_rate_cond[n, :] = sim_rate_cond[n, :] + scipy.stats.norm.rvs(
        scale=0.1, size=len(C)
    )
...
plt.fill(
    list(C) + list(np.flip(C)),
    list(q0025) + list(np.flip(q0975)),
    color="xkcd:grey",
    alpha=0.25,
    label="prior (2.5% - 97.5%)",
)
