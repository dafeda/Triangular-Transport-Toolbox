...
class transport_map:
    ...
    def write_basis_function(self, term, mode="standard", k=None):
        ...
        if mode not in ["standard", "derivative"]:
            raise ValueError(
                "Mode must be either 'standard' or 'derivative'. "
        ...
        if mode == "derivative" and k is None:
            raise ValueError(
                "If mode == 'derivative', specify an integer for k to inform "
                "with regards to which variable we take the derivative. "
                f"Currently specified: k = {k}"
            )
        ...
        # If the entry is a string, it denotes a special term
            ...
            if STtype.lower() == "let":  # The special term is a left edge term
                if mode == "standard":
                    ...
                    string = (
                        f"((__x__[...,{i}] - __mu__)"
                        f"*(1-scipy.special.erf((__x__[...,{i}] - __mu__)"
                        f"/(np.sqrt(2)*__scale__))) - __scale__*np.sqrt(2/np.pi)"
                        f"*np.exp(-((__x__[...,{i}] - __mu__)"
                        f"/(np.sqrt(2)*__scale__))**2))/2"
                    )
            ...
            # -----------------------------------------------------------------
                ...
                if mode == "standard":
                    ...
                    # Construct the string
                    string = (
                        f"np.exp(-((__x__[...,{i}] - __mu__)/__scale__)**2/2)"
                        f"/(__scale__*np.sqrt(2*np.pi))"
                    )
            ...
            # -----------------------------------------------------------------
                ...
                if mode == "standard":
                    ...
                    if int(i) == k:
                        string = (
                            f"1/(np.sqrt(2*np.pi)*__scale__)"
                            f"*np.exp(-(__x__[...,{i}] - __mu__)**2"
                            f"/(2*__scale__**2))"
                        )
            ...
            # -----------------------------------------------------------------
                ...
                raise ValueError(
                    f"Special term '{STtype}' not understood. Currently, "
                    "only LET, RET, iRBF, and RBF are implemented."
                )
        ...
        # Otherwise, it is a standard polynomial term
            ...
            if linearize and self.linearization is None:
                raise Exception(
                    "'LIN' modifier specified in variable monotone, but the "
                    "variable linearization is defined as None. Please specify "
                    "a scalar linearization or remove the 'LIN' modifier."
                )

            # Remove all string-based modifiers
            term = [i for i in term if not isinstance(i, str)]
            ...
            for i in range(len(ui)):
                ...
                # -------------------------------------------------------------
                    ...
                    if hermite_function_modifier:
                        ...
                        string = (
                            f"-1/2*np.exp(-__x__[...,{ui[i]}]**2/4)"
                            f"*(__x__[...,{ui[i]}]*{key} - 2*{keyder})"
                        )
    ...
    def function_constructor_alternative(self, k=None):
        ...
        for k in Ks:
            ...
            if self.linearization is not None:
                # First, find our which parts are outside the linearization
                # hypercube
                string += (
                    "vec_below = copy.copy(x) - "
                    "self.linearization_threshold[:,0][np.newaxis,:];\n\t"
                )
                string += (
                    "vec_below[vec_below >= 0] = 0;\n\t"  # Set values above to zero
                )
                string += (
                    "vec_above = copy.copy(x) - "
                    "self.linearization_threshold[:,1][np.newaxis,:];\n\t"
                )
                string += (
                    "vec_above[vec_above <= 0] = 0;\n\t"  # Set values below to zero
                )
                ...
                # Matrix where entries outside linearization bands are 1
                string += (
                    "shift = np.asarray(below,dtype=float) + "
                    "np.asarray(above,dtype=float);\n\t"
                )

                # Truncate all values outside the hypercube
                string += "x_trc = copy.copy(x);\n\t"
                string += "for d in range(x.shape[1]):\n\t\t"
                # Values below the linearization band snapped to its border
                string += (
                    "x_trc[below[:,d],d] = self.linearization_threshold[d,0];\n\t\t"
                )
                # Values above the linearization band snapped to its border
            ...
            if len(self.nonmonotone[k]) > 0:
                ...
                if self.linearization is not None:
                    ...
                    # Values above the linearization band snapped to its border
                    string += (
                        "x_trc[above[:,d],d] = self.linearization_threshold[d,1];\n\t"
                    )
    ...
    def function_derivative_constructor_alternative(self):
        ...
        for k in range(K):
            ...
            # Are there multiple special terms?
            # if np.sum([True if x != 0 else False for x in
            #     self.RBF_counter_m[k,:]]) > 1:
            # if np.sum([True if x != k else False for x in
            #     list(self.special_terms[k].keys())]) > 1:
            if "cross-terms" in list(
                self.special_terms[k + self.skip_dimensions].keys()
            ):
                import itertools
            ...
            if self.linearization is not None:
                ...
                # hypercube
                string += (
                    "vec_below = self.linearization_threshold[:,0]"
                    "[np.newaxis,:] - x;\n\t"
                )
                string += (
                    "vec_below[vec_below > 0] = 0;\n\t"  # Set values above to zero
                ...
                string += (
                    "vec_above = x - self.linearization_threshold[:,1]"
                    "[np.newaxis,:];\n\t"
                )
                string += (
                    "vec_above[vec_above > 0] = 0;\n\t"  # Set values below to zero
                ...
                string += "above = (vec_above > 0);\n\t"
                string += (
                    "vecnorm = np.asarray(below,dtype=float) + "
                    "np.asarray(above,dtype=float);\n\t"
                )
        ...
        for k in range(self.D):
            ...
            while repeat:
                ...
                for idx, cell in enumerate(proposed_cells):
                    ...
                    coeffs_new = (
                        np.ones(len(nonmonotone) + len(monotone)) * self.coeffs_init
                        + increment
                    )
